...
pub async fn add_exel_table(
    ...
    let mut workbook = open_workbook_auto(file).unwrap();
    let _ = workbook
        .sheet_names()
        .iter()
        .find(|&x| *x == sheet)
    ...
    let (headers, rows) = get_header_rows(range, table_range).await;
    let sheet = deunicode(table_name).to_snake_case();
...
async fn get_header_rows(range: Range<Data>, table_range: &str) -> (Vec<String>, Vec<Vec<Data>>) {
